...
class TemporalGNN(torch.nn.Module):
    def __init__(self, node_features, periods):
        ...
        self.hidden = 2
        # self.tgnn = MSTGCN(nb_block=1, in_channels=node_features, K=1, nb_chev_filter=1, nb_time_filter=1,\
        #      time_strides=1, num_for_predict=self.hidden, len_input=periods)
        self.tgnn = MSTGCN(nb_block=1, in_channels=node_features, K=1, nb_chev_filter=1, nb_time_filter=1,\
             time_strides=1, num_for_predict=self.hidden, len_input=periods)

        self.linear = torch.nn.Linear(self.hidden, periods)
        self.train_year = 5
...
def train():
    ...
    for epoch in bar:
        ...
        feat = feature.reshape(1, -1, 4, 5).to(device)
